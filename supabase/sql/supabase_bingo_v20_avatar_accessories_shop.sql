-- V20: Avatar accessories shop — add XP costs to head accessory items
-- Replaces get_avatar_item_cost() to recognise acc_* keys.
-- purchase_avatar_item() from V17 already handles acc_* keys correctly
-- once this function returns non-null costs for them — no RPC changes needed.
...
as $$
  select case p_item_key
    -- Head / face shapes (unchanged from V18)
