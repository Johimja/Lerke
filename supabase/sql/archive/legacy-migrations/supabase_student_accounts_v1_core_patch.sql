-- Lerke
-- Student accounts / classes V1 core patch
--
-- Canonical student-accounts rollout patch for an existing Bingo V1 database.
-- Adds teacher-managed classes, students, and class code + student code + PIN
-- login scaffolding.
--
-- This file now includes the later RLS and crypto fixes as well.
-- Keep the smaller follow-up patches only for databases that already ran an
-- earlier version of this rollout.
--
-- This patch is designed to coexist with the current anonymous Bingo flow.
...
drop policy if exists student_profiles_student_select_self on public.student_profiles;
create policy student_profiles_student_select_self
on public.student_profiles
for select
to authenticated
using (
  exists (
    select 1
    from public.student_auth_links sal
    where sal.student_id = id
      and sal.auth_user_id = auth.uid()
  )
);

drop policy if exists student_auth_links_select_self on public.student_auth_links;
create policy student_auth_links_select_self
on public.student_auth_links
for select
to authenticated
using (auth_user_id = auth.uid());

-- No direct student credential select policy.
-- PIN material should only be handled through RPCs.
...
revoke all on function public.list_class_students(uuid) from public;
revoke all on function public.reset_student_pin(uuid) from public;
revoke all on function public.student_login_with_pin(text, text, text) from public;
...
grant execute on function public.student_login_with_pin(text, text, text) to authenticated;
