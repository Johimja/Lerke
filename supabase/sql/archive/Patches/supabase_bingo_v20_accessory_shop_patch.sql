-- =========================================================
-- V20: Head accessory shop — XP costs (Avatar-8)
-- =========================================================
-- All accessories were introduced free (xp:0) in Avatar-7 (head accessory
-- layer). This patch assigns real XP costs to 19 of the 20 accessories
-- (acc_none stays free) and re-wires get_avatar_item_cost() so the existing
-- purchase_avatar_item(p_item_key) RPC works for acc_* keys without any
...
as $$
  select case p_item_key
    ...
    when 'acc_earmuffs'        then 90
    when 'acc_graduation'      then 100
    when 'acc_cowboy'          then 110
    when 'acc_chef_hat'        then 110
