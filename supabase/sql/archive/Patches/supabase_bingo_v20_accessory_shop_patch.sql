-- =========================================================
-- Avatar-8: Head Accessories Shop — XP Cost Extension
-- Patch for databases that already have v17 (avatar_shop) applied.
-- Extends get_avatar_item_cost() to include all acc_* keys.
-- purchase_avatar_item() is unchanged — it already routes through
-- get_avatar_item_cost() and handles xp deduction + unlocked_avatar_items.
-- =========================================================

-- Replace the immutable cost helper with extended version
...
as $$
  select case p_item_key
    -- Head / face shapes (v17 original values)
    ...
    when 'acc_chef_hat'        then 175
    when 'acc_sombrero'        then 175
    when 'acc_earmuffs'        then 200
    when 'acc_tiara'           then 200
    when 'acc_bunny_ears'      then 225
