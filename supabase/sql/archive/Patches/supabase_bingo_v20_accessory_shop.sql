-- Avatar-8: Head accessories shop XP costs
-- Extends get_avatar_item_cost() to recognise acc_* keys.
-- purchase_avatar_item() already routes through this function, so no other
-- SQL changes are needed — accessories are purchased/unlocked the same way
-- as head items.
...
as $$
  select case p_item_key
    -- Head silhouette items (v17 / v18 catalogue)
    ...
    else null  -- unknown item → purchase_avatar_item returns {ok:false}
  end;
$$;
