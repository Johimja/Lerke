-- =========================================================
-- V20 patch: Avatar-8 — head accessory XP costs
-- Apply to any DB that has v17/v18 avatar shop (get_avatar_item_cost,
-- purchase_avatar_item, unlocked_avatar_items already exist).
-- =========================================================
--
-- Avatar-7 added the 20-item ACCESSORY_CATALOGUE (acc_* keys) as a free
-- second sprite layer (xp:0). This patch assigns real XP costs so the
-- existing purchase_avatar_item RPC can sell them like head items.
-- The RPC and unlocked_avatar_items column already support arbitrary
-- item keys — only the cost lookup needs the new acc_* branches.
-- =========================================================
...
as $$
  select case p_item_key
    ...
    when 'acc_sombrero'        then 200
    when 'acc_tophat'          then 200
    when 'acc_viking'          then 225
    when 'acc_laurel'          then 225
