...
begin
  ...
  if found then
    ...
    set client_token = p_client_token,
        last_seen_at = now(),
        status = 'active'
...
-- Important:
-- The policies below are intentionally teacher-first.
-- They are sufficient for authenticated teacher clients,
-- but not sufficient yet for anonymous student realtime subscriptions.
--
-- For student-side live reads, V1 uses Supabase anonymous auth
-- with scoped student-readable policies and RPC functions.
--
-- This draft keeps student write access closed by default.
...
-- Intentionally no self-update policy here.
...
-- Note:
-- Student-side participant insert/update remains closed for direct table access.
-- The intended write path is via RPC:
-- - join_bingo_session()
-- - reroll_nickname()
-- - touch_participant()
