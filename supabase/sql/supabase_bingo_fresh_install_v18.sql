...
begin
  ...
  if v_current_color is not distinct from p_color then
    return jsonb_build_object('ok', true, 'xp_spent', 0, 'total_xp', v_current_xp, 'avatar_data', v_avatar_data);
  end if;
  ...
  return jsonb_build_object('ok', true, 'xp_spent', v_cost, 'total_xp', v_current_xp - v_cost, 'avatar_data', v_avatar_data);
end;
$$;
