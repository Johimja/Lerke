...
begin
  ...
  if (v_current_avatar ->> p_color_slot) = p_color then
    return jsonb_build_object('ok', true, 'no_change', true,
      'total_xp', v_current_xp, 'avatar_data', v_current_avatar, 'xp_spent', 0);
  end if;
  if coalesce(v_current_xp, 0) < v_cost then
    return jsonb_build_object('ok', false, 'error', 'insufficient_xp',
      'required', v_cost, 'available', coalesce(v_current_xp, 0));
  end if;
  ...
  where  id = v_student_id;
  return jsonb_build_object('ok', true, 'total_xp', v_current_xp - v_cost,
    'avatar_data', v_new_avatar, 'xp_spent', v_cost);
end;
$$;
