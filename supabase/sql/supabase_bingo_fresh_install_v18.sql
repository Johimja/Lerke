...
as $$
  select case p_item_key
    -- Head face-shapes (v18_avatar_faceshapes)
    when 'head_basic'          then 0
    when 'head_flat_top'       then 0
    when 'head_widows_peak'    then 50
    when 'head_crew'           then 50
    ...
    when 'head_cap'            then 175
    when 'head_flat_top_beard' then 300
    -- Head accessories (v20_avatar_accessories)
...
-- V20: Avatar head accessories XP costs are included in the
-- get_avatar_item_cost() function above (acc_* keys added inline).
-- No additional statements required for a fresh install.
