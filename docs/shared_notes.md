...
### 2026-07-20 — Automated: Paid skin color changes (Avatar-9)
...
- Created `supabase/sql/archive/Patches/supabase_bingo_v21_avatar_color_patch.sql`:
  ...
  - **Migration applied** ✅ (`v21_avatar_color` via Supabase MCP, 2026-07-20, project `isuzuuvddteejktcowev`).
...
- `index.html`:
  ...
  - Added CSS for `.avatar-color-tab`, `.avatar-color-row`, `.avatar-color-label`, `.avatar-color-swatch`, `.avatar-color-hint`, with dark-mode overrides.
...
- Tests:
  - Added `tests/avatar_color.test.mjs` — 23 assertions covering `normalizeAvatarData`, skin layer CSS, Farge tab, RPC call, teacher rendering, SQL content, and fresh install.

**Verification run:**

- `node tests/avatar_color.test.mjs` ✅ (23 passed)
- All other tests ✅
- JS syntax check (both HTML files) ✅

**Next task:** Avatar-10+ — hair sheet (generate ~20 hair silhouettes and integrate as a new layer above the face shape). Or phase out anonymous join (Tier 1).
