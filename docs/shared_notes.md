...
### 2026-06-19 — Automated: Head accessory XP costs (Avatar-8)

**What was done:**

- `index.html`: `ACCESSORY_CATALOGUE` now has real XP costs per accessory (previously all `xp:0`). `acc_none` stays free; costs range 50–300 XP based on rarity/flashiness (e.g. `acc_party_hat`=50, `acc_viking`=200, `acc_crown`=300). No other frontend changes needed — `shopItemClick`/`purchaseAndEquipItem`/`_itemByKey` in `index.html` already handled any catalogue item generically via the existing `purchase_avatar_item` RPC, since Avatar-7's layered renderer refactor.
- SQL: Created and applied `supabase/sql/supabase_bingo_v20_avatar_accessory_costs.sql` — extends `get_avatar_item_cost()` (previously only recognized `head_*` keys) with all 20 `acc_*` keys and matching XP costs. `purchase_avatar_item()` itself needed no changes; it already handles any key via `get_avatar_item_cost()`.
- `supabase/sql/supabase_bingo_fresh_install_v18.sql` updated in place (the `get_avatar_item_cost` definition block) so new installs get accessory costs without needing a separate patch.
- `apps/bingo/teacher.html`'s `ACCESSORY_CATALOGUE_T` is display-only (col/row, no xp/purchase logic) — left unchanged.
- Tests: Added `tests/avatar_accessory_costs.test.mjs` — asserts the SQL `acc_*` cost cases exactly match the `index.html` catalogue's `xp` values (would catch accidental drift in the future), and that `acc_none` is free.

**Migration applied** ✅ (`v20_avatar_accessory_costs` via Supabase MCP on project `isuzuuvddteejktcowev`, 2026-06-19). Verified `acc_crown=300`, `acc_none=0`, invalid key returns null, `head_basic` unaffected.
...
**Next task:** Avatar-9 — paid color changes (color picker/slider + server-verified XP cost per saved color change). Or Avatar-10+ (hair/eyes/beards/mouths sheets).
...
- [x] **Avatar-8: head accessories shop** — added XP costs to accessories (`index.html` `ACCESSORY_CATALOGUE`), SQL v20 extends `get_avatar_item_cost()` with `acc_*` keys; existing `purchase_avatar_item` RPC and frontend purchase flow already worked generically for any catalogue item. ✅
...
20. `supabase/sql/supabase_bingo_v20_avatar_accessory_costs.sql` ✅ applied
