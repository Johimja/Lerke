...
### 2026-06-03 — Automated: Avatar-8 — head accessories shop (XP costs)

**What was done:**

- `index.html`:
  - Updated `ACCESSORY_CATALOGUE` — all 19 paid accessories now have real XP costs (acc_none stays free). Tier breakdown: headband/beanie 50 XP; earmuffs/bow/cap/bunny_ears/bandana 75 XP; party_hat/graduation/chef_hat/antlers 100 XP; laurel/cowboy/sombrero 125 XP; tophat/tiara/witch_hat 150 XP; crown/viking 200 XP.
  - No logic changes needed — `shopItemClick` already routes paid items through `purchaseAndEquipItem → purchase_avatar_item RPC`, which calls `get_avatar_item_cost` to validate cost and deduct XP.
- SQL:
  - Created and applied `supabase/sql/archive/Patches/supabase_bingo_v20_accessory_shop.sql` — replaces `get_avatar_item_cost()` with a version that handles all 20 `acc_*` keys in addition to the existing 20 `head_*` keys.
  - Updated `supabase/sql/supabase_bingo_fresh_install_v18.sql` to include `acc_*` costs in its embedded `get_avatar_item_cost` definition (fresh installs will get the full catalogue).
- Migration applied ✅ (`v20_accessory_shop` via Supabase MCP, 2026-06-03).

**Next task:** Avatar-9 — paid color changes (SQL RPC for color-change purchases, UI color picker, per-save XP deduction, canvas/CSS recoloring).
...
- [x] **Avatar-8: head accessories shop** — added XP costs to all 19 paid accessories in `ACCESSORY_CATALOGUE` (index.html); updated `get_avatar_item_cost()` to recognise all `acc_*` keys; existing `purchase_avatar_item` RPC already handles cost validation/deduction/unlock for any key this helper returns a value for. SQL migration `v20_accessory_shop` applied. ✅
